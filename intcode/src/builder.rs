...
impl Builder {
    ...
    pub fn program(&mut self, program: &Vec<i32>) -> &mut Self {
    ...
    pub fn init_hook(&mut self, hook: &Vec<i32>) -> &mut Self {
        self._init_hook = Some(hook.clone());
        self
    }

    pub fn exit_hook(&mut self, hook: &Vec<i32>) -> &mut Self {
        self._exit_hook = Some(hook.clone());
        self
    }

    pub fn run(&self, input: Receiver<i32>, output: Sender<i32>) {
    ...
    pub fn run_noninteractive(&self, input: &mut dyn Iterator<Item = i32>) -> Vec<i32> {
